...
There are two ways to build a calculator. Choose the one that fits your tool:

The Automatic Way (Recommended)

Use for: 90% of tools (BMI, Age, Converters).

Function: CalculatorLayout.render(...)

How it works: You just create two hidden divs (tool-slot and seo-slot). The engine automatically builds the breadcrumbs, title, grid, sidebar, and footer for you.

The Manual Way (Advanced)

Use for: Complex tools (e.g., Mortgage Calculator, Charts).

Function: loadCommonLayout()

How it works: You write the entire HTML grid yourself. You only call loadCommonLayout() to inject the Header and Footer.
